...
const oldPlayMacro = /function playMacroItem\(mIndex, q, direction = 1\) \{[\s\S]*?(?=function getCurrentMacroIndex)/m;
...
let match = code.match(oldPlayMacro);
if (match) {
   code = code.replace(oldPlayMacro, newPlayMacro);
   fs.writeFileSync('index.html', code);
   console.log("Patched playMacroItem explicitly");
} else {
   console.log("Could not find playMacroItem");
}
